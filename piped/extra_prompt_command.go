...
type DoubleBaseCommand interface {
	Flags() []cli.Flag
	PreparePromptForDoubleAction(cliContext *cli.Context, storedPrompt string) (string, error)
	Name() string
	Usage() string
}
...
func (c *DoublePromptCommand) Usage() string {
	return "Running prompt in double mode. Means you give a topic. Jess generate basic prompt and generate 2 files - one with generated prompt for future edit int and second with the result of the prompt."
}
...
func (c *DoublePromptCommand) PreparePromptForDoubleAction(cliContext *cli.Context, storedPrompt string) (string, error) {
